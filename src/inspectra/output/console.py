...
def print_results(results: list[ReviewResult], pr_summary: str = "") -> None:
    ...
    if not total_issues:
        console.print(Panel("[bold green]✅ No issues found — the diff looks great![/bold green]", border_style="green"))
        return
    ...
    for result in results:
        ...
        for issue in sorted(result.issues, key=lambda i: _severity_sort_key(i.severity)):
            ...
            if issue.rule_id:
                lines.append(f"[bold]Rule:[/bold] {issue.rule_id}")
...
def print_stats(results: list[ReviewResult]) -> None:
    ...
    console.print(
        ...
        f"{total} issue(s) · [red]{critical} critical[/red] · [orange1]{high} high[/orange1]"
    )
